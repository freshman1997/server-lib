...
namespace
{
    class InteractivePasswordHandler final : public SshHandler
    {
    public:
...
int main()
{
    const char * smoke_env = std::getenv("YUAN_RUN_RELEASE_SSH_CLI_INTERACTIVE_SMOKE");
    if (!smoke_env || std::string(smoke_env) != "1") {
        std::cout << "release_ssh_cli interactive smoke skipped (set YUAN_RUN_RELEASE_SSH_CLI_INTERACTIVE_SMOKE=1)." << std::endl;
        return 0;
    }

    const auto cli_bin = find_release_cli_binary();
    if (cli_bin.empty()) {
        std::cout << "release_ssh_cli binary not found; skipping interactive smoke." << std::endl;
        return 0;
    }

    const auto smoke_dir = std::filesystem::current_path() / ".tmp_release_ssh_cli_interactive_smoke";
    std::error_code ec;
    ...
    const auto out_file = smoke_dir / "interactive_out.txt";
    ...
    InteractivePasswordHandler handler;
    auto server = std::make_unique<SshServer>(config);
    server->set_handler(&handler);

    constexpr int port = 22431;
    if (!server->init(port)) {
    ...
    const std::string input_script = "printf release-cli-interactive-ok\\n\\nexit\\n";
    std::string cmd = "printf " + shell_quote(input_script) + " | " +
    ...
    if (status != 0) {
        std::cerr << "release_ssh_cli interactive command failed with status " << status << std::endl;
        return 1;
    }
    if (!file_contains(out_file, "release-cli-interactive-ok")) {
        std::cerr << "release_ssh_cli interactive output mismatch" << std::endl;
        return 1;
    }

    std::cout << "release_ssh_cli interactive smoke passed." << std::endl;
    return 0;
}
