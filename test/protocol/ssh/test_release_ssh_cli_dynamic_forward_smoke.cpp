...
namespace
{
    ...
    class DynamicForwardPasswordHandler final : public SshHandler
    {
    public:
        ...
        bool on_direct_tcpip(SshSession *,
                             SshChannel *,
                             const std::string &,
                             uint16_t) override
        {
...
int main()
{
    const char * smoke_env = std::getenv("YUAN_RUN_RELEASE_SSH_CLI_DYNAMIC_FORWARD_SMOKE");
    if (!smoke_env || std::string(smoke_env) != "1") {
        std::cout << "release_ssh_cli dynamic-forward smoke skipped (set YUAN_RUN_RELEASE_SSH_CLI_DYNAMIC_FORWARD_SMOKE=1)." << std::endl;
        return 0;
    }
    ...
    if (cli_bin.empty()) {
        std::cout << "release_ssh_cli binary not found; skipping dynamic-forward smoke." << std::endl;
#ifdef _WIN32
        WSACleanup();
#endif
        return 0;
    ...
    const auto smoke_dir = std::filesystem::current_path() / ".tmp_release_ssh_cli_dynamic_forward_smoke";
    std::error_code ec;
    ...
    constexpr int server_port = 22438;
    constexpr int socks_port = 22439;
    constexpr int target_port = 22440;
    ...
    DynamicForwardPasswordHandler handler;
    ...
    std::thread echo_thread([&]() {
        ...
        if (n > 0) {
            ...
            const std::string want = "dyn-smoke\n";
            if (got == want) {
                const std::string reply = "dyn-smoke-reply\n";
    ...
    std::string cli_cmd = shell_quote(cli_bin.string()) +
                          ...
                          " -D 127.0.0.1:" + std::to_string(socks_port) +
    ...
    for (int i = 0; i < 150 && !cli_done.load(std::memory_order_relaxed); ++i) {
        ...
        SocketFd probe = connect_once("127.0.0.1", static_cast<uint16_t>(socks_port));
        ...
        if (!send(probe, reinterpret_cast<const char *>(greeting.data()), static_cast<int>(greeting.size()), 0)) {
            close_socket(probe);
            continue;
        }
        ...
        std::array<uint8_t, 10> connect_req = {
            ...
            0,
            1,
            static_cast<uint8_t>((target_port >> 8) & 0xFF),
            static_cast<uint8_t>(target_port & 0xFF)
        };
        if (!send(probe, reinterpret_cast<const char *>(connect_req.data()), static_cast<int>(connect_req.size()), 0)) {
            close_socket(probe);
            continue;
        }
        ...
        const std::string line = "dyn-smoke\n";
        ...
        close_socket(probe);
        if (n > 0) {
            const std::string got(data_buf.data(), data_buf.data() + n);
            if (got == "dyn-smoke-reply\n") {
...
#endif
    if (status != 0) {
        std::cerr << "release_ssh_cli dynamic-forward command failed with status " << cli_status.load() << std::endl;
#ifdef _WIN32
        WSACleanup();
#endif
        return 1;
    ...
    if (!echo_ok.load(std::memory_order_relaxed) || !forwarded_exchange_ok) {
        std::cerr << "release_ssh_cli dynamic-forward data path failed" << std::endl;
#ifdef _WIN32
        WSACleanup();
#endif
        return 1;
    ...
    std::cout << "release_ssh_cli dynamic-forward smoke passed." << std::endl;
#ifdef _WIN32
    WSACleanup();
#endif
    return 0;
