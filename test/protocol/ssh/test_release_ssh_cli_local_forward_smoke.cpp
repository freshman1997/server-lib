...
namespace
{
    ...
    class LocalForwardPasswordHandler final : public SshHandler
    {
    public:
...
int main()
{
    const char * smoke_env = std::getenv("YUAN_RUN_RELEASE_SSH_CLI_LOCAL_FORWARD_SMOKE");
    if (!smoke_env || std::string(smoke_env) != "1") {
        std::cout << "release_ssh_cli local-forward smoke skipped (set YUAN_RUN_RELEASE_SSH_CLI_LOCAL_FORWARD_SMOKE=1)." << std::endl;
        return 0;
    }
    ...
    if (cli_bin.empty()) {
        std::cout << "release_ssh_cli binary not found; skipping local-forward smoke." << std::endl;
#ifdef _WIN32
        WSACleanup();
#endif
        return 0;
    ...
    const auto smoke_dir = std::filesystem::current_path() / ".tmp_release_ssh_cli_local_forward_smoke";
    std::error_code ec;
    ...
    constexpr int server_port = 22435;
    constexpr int forward_port = 22436;
    constexpr int target_port = 22437;
    ...
    LocalForwardPasswordHandler handler;
    ...
    std::thread echo_thread([&]() {
        ...
        if (n > 0) {
            ...
            const std::string want = "lf-smoke\n";
            if (got == want) {
                const std::string reply = "lf-smoke-reply\n";
    ...
    for (int i = 0; i < 150 && !cli_done.load(std::memory_order_relaxed); ++i) {
        ...
        const std::string line = "lf-smoke\n";
...
#endif
    if (status != 0) {
        std::cerr << "release_ssh_cli local-forward command failed with status " << cli_status.load() << std::endl;
#ifdef _WIN32
        WSACleanup();
#endif
        return 1;
    ...
    if (!echo_ok.load(std::memory_order_relaxed) || !forwarded_exchange_ok) {
        std::cerr << "release_ssh_cli local-forward data path failed" << std::endl;
#ifdef _WIN32
        WSACleanup();
#endif
        return 1;
    ...
    std::cout << "release_ssh_cli local-forward smoke passed." << std::endl;
#ifdef _WIN32
    WSACleanup();
#endif
    return 0;
