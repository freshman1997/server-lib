...
public:
    bool connect(const std::string& host, int port = 21)
    {
        ...
        if (client_.connect(host, static_cast<uint16_t>(port))) {
            connected_ = true;
    ...
    bool list(const std::string& path = "")
    {
        ...
        const std::string output = client_.list(path);
        if (!output.empty()) {
    ...
    bool nlist(const std::string& path = "")
    {
        ...
        const std::string output = client_.nlist(path);
        if (!output.empty()) {
            std::cout << output << std::endl;
            return true;
        }
...
void print_help()
{
    ...
    std::cout << "  connect 127.0.0.1 21" << std::endl;
...
int main()
{
    ...
    std::string input;
    while (true) {
        std::cout << (client.is_logged_in() ? "ftp> " : "ftp-not-auth> ");
