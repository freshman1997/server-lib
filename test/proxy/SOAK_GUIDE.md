...
```powershell
cmake --build build --target proxy_server -j 4
```
...
```powershell
.\test\proxy\run_proxy_soak.ps1 `
  -ProxyServerPath .\build\test\proxy\proxy_server.exe `
  -ProxyPort 3128 `
  -DurationSec 1800 `
  -Concurrency 8
```
...
```powershell
.\test\proxy\analyze_proxy_soak.ps1 -ProxyLog .\logs\proxy_soak\proxy-<timestamp>.log -IdleTailSeconds 30
```
...
```powershell
.\test\proxy\run_proxy_stream_soak.ps1 `
  -ProxyServerPath .\build\test\proxy\proxy_server.exe `
  -ProxyPort 3128 `
  -UpstreamPort 18080 `
  -DurationSec 1800 `
...
```powershell
.\test\proxy\analyze_proxy_soak.ps1 -ProxyLog .\logs\proxy_stream_soak\proxy-<timestamp>.log -IdleTailSeconds 30
```
