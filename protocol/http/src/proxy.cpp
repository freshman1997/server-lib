...
size_t TargetConnectionPool::active_count() const
{
    ...
    for (const auto &pc : connections_) {
        if (pc.in_use.load(std::memory_order_relaxed)) ++n;
    }
    return n;
}
...
{
    if (!server || !proxy) return nullptr;
    ...
    server->get_event_loop()->on_new_connection(conn);
    conn->set_connection_handler(proxy);
    conn->set_event_handler(server->get_event_loop());
...
HttpProxy::~HttpProxy()
{
    std::vector<Connection *> mapped_connections;
    {
        ...
        for (auto &pair : sc_mapping_) {
            if (pair.first) mapped_connections.push_back(pair.first);
            if (pair.second.client_conn) mapped_connections.push_back(pair.second.client_conn);
        }
        sc_mapping_.clear();
        cs_mapping_.clear();
...
std::string HttpProxy::find_proxy_route(const std::string &url) const
{
    ...
    if (!result || !result.is_registered) return "";
...
void HttpProxy::serve_proxy(HttpRequest *req, HttpResponse *resp)
{
    ...
    if (server_ && server_->get_timer_manager()) {
        auto weak_task = std::weak_ptr<RemoteConnectTask>(task);
        auto *timer = timer::TimerUtil::build_timeout_timer(
            server_->get_timer_manager(),
            static_cast<uint64_t>(route.connect_timeout_ms),
            [this, weak_task](timer::Timer *) {
...
{
    if (route.targets.empty()) return ProxyTarget{};

    switch (route.balance) {
        case ProxyRoute::BalanceStrategy::round_robin:
        {
            auto &idx = rr_indices_[route.match_pattern];
            size_t i = idx.fetch_add(1, std::memory_order_relaxed) % route.targets.size();
...
{
    int total_weight = 0;
    for (const auto &t : targets) total_weight += (std::max)(1, t.weight);
    ...
    for (const auto &t : targets) {
        ...
        if (rand_val <= cumulative) return t;
    }
    return targets.back();
}
                                      ...
                                      const ProxyTarget &target)
{
    ...
    // 注意：路径重写在serve_proxy中由路由匹配完成后自然生成    // 如果需要显式重写URL path，可以在这里处理
    if (route.strip_prefix && !route.rewrite_prefix.empty()) {
        // 将原始URL中的匹配前缀替换为新的前缀
        // 这需要在pack_and_send前修改请求行中的path
        // 由于当前HttpRequest结构限制，这里仅做标记       // 实际改写在序列化阶段处理
    }
}
...
Connection *HttpProxy::create_remote_connection(const ProxyTarget &target, int timeout_ms)
{
    ...
    if (server_ && server_->get_event_loop()) {
                                         ...
                                         const ProxyRoute &route, const ProxyTarget &target)
{
    ...
    if (upgrade_lower != "websocket") return false;

    // WebSocket升级走正常的代理流程即可
    // 因为WebSocket握手后就是TCP长连接双向转发了，不需要特殊处理
    serve_proxy(req, resp);
    return true;
}
...
void HttpProxy::cleanup_idle_connections()
{
    std::vector<std::shared_ptr<TargetConnectionPool>> pool_snapshot;
    {
        ...
        pool_snapshot.reserve(pools_.size());
        for (auto &pair : pools_) {
            if (pair.second) pool_snapshot.push_back(pair.second);
        }
    }
