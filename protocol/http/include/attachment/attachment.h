...
namespace yuan::net::http
{
    struct AttachmentInfo
    {
        ...
        AttachmentInfo(const std::string &tmpFileName, const std::string &originFileName, const std::string &contentType, std::size_t offset, std::size_t length)
            : tmp_file_name_(tmpFileName), origin_file_name_(originFileName), content_type_(contentType), source_offset_(0), offset_(offset), length_(length) {}
        
        AttachmentInfo(const AttachmentInfo &other)
            : tmp_file_name_(other.tmp_file_name_), origin_file_name_(other.origin_file_name_), content_type_(other.content_type_), source_offset_(other.source_offset_), offset_(other.offset_), length_(other.length_) {}
        
        AttachmentInfo(AttachmentInfo &&other) noexcept
            : tmp_file_name_(std::move(other.tmp_file_name_)), origin_file_name_(std::move(other.origin_file_name_)), content_type_(std::move(other.content_type_)), source_offset_(other.source_offset_), offset_(other.offset_), length_(other.length_)
        {
            other.source_offset_ = 0;
            other.offset_ = 0;
            other.length_ = 0;
        }
