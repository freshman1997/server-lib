...
namespace yuan::net::bit_torrent
{
    ...
    struct HandshakeMessage
    {
        ...
        HandshakeMessage()
        {
            ...
            reserved_[7] |= 0x04; // Fast extension (BEP 6)
            reserved_[5] |= 0x10; // Extension protocol (BEP 10)
        }
        ...
        bool deserialize(const uint8_t *data, size_t len)
        {
            if (len < HANDSHAKE_SIZE)
                return false;
            if (data[0] != PROTOCOL_STR_LEN)
                return false;
            if (std::memcmp(data + 1, PROTOCOL_STR, PROTOCOL_STR_LEN) != 0)
                return false;
        ...
        bool supports_extension() const
        {
            return (reserved_[5] & 0x10) != 0;
        }
        bool supports_dht() const
        {
            return (reserved_[7] & 0x01) != 0;
        }
        bool supports_fast() const
        {
            return (reserved_[7] & 0x04) != 0;
        }
    };
    ...
    struct PeerMessage
    {
        ...
        static PeerMessage suggest_piece(uint32_t piece_index)
        {
            PeerMessage msg;
            msg.length_ = 5;
            msg.id_ = PeerMessageId::suggest_piece;
            uint8_t payload[4];
            payload[0] = (piece_index >> 24) & 0xFF;
            payload[1] = (piece_index >> 16) & 0xFF;
            payload[2] = (piece_index >> 8) & 0xFF;
        ...
        static PeerMessage have_all()
        {
            PeerMessage msg;
            msg.length_ = 1;
            msg.id_ = PeerMessageId::have_all;
            return msg;
        }

        static PeerMessage have_none()
        {
            PeerMessage msg;
            msg.length_ = 1;
            msg.id_ = PeerMessageId::have_none;
            return msg;
        }

        static PeerMessage reject_request(uint32_t piece_index, uint32_t offset, uint32_t length)
        {
            PeerMessage msg;
            msg.length_ = 13;
            msg.id_ = PeerMessageId::reject_request;
            uint8_t payload[12];
            payload[0] = (piece_index >> 24) & 0xFF;
            payload[1] = (piece_index >> 16) & 0xFF;
            payload[2] = (piece_index >> 8) & 0xFF;
        ...
        static PeerMessage allowed_fast(uint32_t piece_index)
        {
            PeerMessage msg;
            msg.length_ = 5;
            msg.id_ = PeerMessageId::allowed_fast;
            uint8_t payload[4];
            payload[0] = (piece_index >> 24) & 0xFF;
            payload[1] = (piece_index >> 16) & 0xFF;
            payload[2] = (piece_index >> 8) & 0xFF;
        ...
        static PeerMessage extended(uint8_t ext_id, const uint8_t *payload, size_t payload_len)
        {
            PeerMessage msg;
            msg.length_ = static_cast<uint32_t>(1 + 1 + payload_len);
            msg.id_ = PeerMessageId::extended;
            msg.payload_.push_back(ext_id);
            if (payload_len > 0 && payload)
                msg.payload_.insert(msg.payload_.end(), payload, payload + payload_len);
            return msg;
        }
        ...
        {
            if (id_ != PeerMessageId::have || payload_.size() < 4)
                return 0;
        ...
        uint8_t extended_id() const
        {
            if (id_ != PeerMessageId::extended || payload_.empty())
                return 0;
            return payload_[0];
        }

        const uint8_t *extended_payload() const
        {
            if (id_ != PeerMessageId::extended || payload_.size() < 2)
                return nullptr;
            return payload_.data() + 1;
        }

        size_t extended_payload_size() const
        {
            if (id_ != PeerMessageId::extended || payload_.size() < 2)
                return 0;
        ...
        uint16_t port_value() const
        {
            if (id_ != PeerMessageId::port || payload_.size() < 2)
                return 0;
            return (static_cast<uint16_t>(payload_[0]) << 8) | payload_[1];
        }

        uint32_t suggest_piece_index() const
        {
            if (id_ != PeerMessageId::suggest_piece || payload_.size() < 4)
                return 0;
            return (static_cast<uint32_t>(payload_[0]) << 24) |
                   (static_cast<uint32_t>(payload_[1]) << 16) |
                   (static_cast<uint32_t>(payload_[2]) << 8) |
        ...
        uint32_t allowed_fast_index() const
        {
            if (id_ != PeerMessageId::allowed_fast || payload_.size() < 4)
                return 0;
            return (static_cast<uint32_t>(payload_[0]) << 24) |
                   (static_cast<uint32_t>(payload_[1]) << 16) |
                   (static_cast<uint32_t>(payload_[2]) << 8) |
        ...
        uint32_t reject_piece_index() const
        {
            if (payload_.size() < 4)
                return 0;
            return (static_cast<uint32_t>(payload_[0]) << 24) |
    ...
    struct PeerState
    {
        ...
        void set_have_all(int32_t total_pieces)
        {
            pieces.assign(total_pieces, true);
        }

        void set_have_none(int32_t total_pieces)
        {
            pieces.assign(total_pieces, false);
        }
