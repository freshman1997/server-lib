...
// Set receive timeout
    ...
    bool UpnpManager::parse_igd_services(const std::string & xml)
    {
        ...
        if (service_tag == std::string::npos)
            return false;
    ...
    bool UpnpManager::soap_add_port_mapping()
    {
        ...
        std::string body =
            ...
            protocol + "</NewProtocol>\r\n"
                       "      <NewInternalPort>" +
            int_port + "</NewInternalPort>\r\n"
                       "      <NewInternalClient>" +
            local_ip + "</NewInternalClient>\r\n"
                       "      <NewEnabled>1</NewEnabled>\r\n"
                       "      <NewPortMappingDescription>" +
            desc + "</NewPortMappingDescription>\r\n"
                   "      <NewLeaseDuration>" +
            std::to_string(config_.upnp_lease_duration) + "</NewLeaseDuration>\r\n"
                                                          "    </u:AddPortMapping>\r\n"
                                                          "  </s:Body>\r\n"
                                                          "</s:Envelope>\r\n";
    ...
    {
        uint32_t lease_seconds = config_.upnp_lease_duration > 0
                                     ? static_cast<uint32_t>(config_.upnp_lease_duration)
                                     : 3600;
        uint32_t renew_interval = lease_seconds / 2;
        if (renew_interval < 60)
            renew_interval = 60;

        while (running_.load()) {
            for (uint32_t i = 0; i < renew_interval && running_.load(); ++i) {
