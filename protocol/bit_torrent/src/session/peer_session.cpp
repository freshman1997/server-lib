...
{
    ev_loop_ = config.ev_loop_;
    timer_manager_ = config.timer_manager_;
...
void PeerSession::set_runtime(net::EventLoop *loop, timer::TimerManager *timer_manager)
{
    ev_loop_ = loop;
...
void PeerSession::attach_peer(PeerConnection *peer, const std::string &key)
{
    if (!peer)
    {
        return;
...
{
    if (!meta_ || !peer_id_ || !pieces_have_ || !ev_loop_ || !timer_manager_)
    {
        return;
    }
...
void PeerSession::on_inbound_peer(PeerConnection *peer)
{
