...
{
    FtpCommandResponse CommandNlist::execute(FtpSession * session, const std::string & args)
    {
        ...
        info.memory_content_ = build_nlist_payload(target);
        ...
        return { FtpResponseCode::__150__, "Opening data connection for NLST." };
    }
    CommandType CommandNlist::get_command_type()
    {
        return CommandType::cmd_nlist;
    }
    std::string CommandNlist::get_command_name()
    {
        return "NLST";
    }
}
