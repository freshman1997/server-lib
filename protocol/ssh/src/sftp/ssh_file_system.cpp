...
namespace yuan::net::ssh
{
    namespace
    {
        ...
        {
            return yuan::platform::GetLastSystemError();
        }
    ...
    SshFsOpenResult SshLocalFileSystem::open(const std::string & path, uint32_t pflags, const SftpFileAttrs &)
    {
        ...
        if (!exists && want_create) {
            ...
            if (!create_file) {
                ...
                result.status_message = yuan::platform::DescribeNativeError(err);
                return result;
            }
    ...
    SshFsWriteResult SshLocalFileSystem::write(const std::string & handle, uint64_t offset, const uint8_t * data, uint32_t len)
    {
        ...
        if (written != len) {
            const int err = last_fs_error();
            result.status = errno_to_status(err);
            result.status_message = written == 0 ? yuan::platform::DescribeNativeError(err) : "Short write";
            return result;
        }
