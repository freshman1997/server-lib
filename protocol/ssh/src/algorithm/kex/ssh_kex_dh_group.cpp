#include "algorithm/ssh_kex_algorithm.h"
#include "crypto/ssh_crypto.h"
#include "crypto/ssh_key_derivation.h"
#include <cstring>
#include <memory>

namespace yuan::net::ssh
{
    static const uint8_t dh_group14_prime[] = {
        ,
    };

    static const uint8_t dh_group16_prime[] = {
        
    };

    static const uint8_t dh_group18_prime[] = {
        
    };

    static const uint8_t dh_generator[] = { 0x02 };

    class SshKexDhGroup14Sha256 : public SshKexAlgorithm
    {
    public:
        std::string name() const override
        {
            return "diffie-hellman-group14-sha256";
        }
        size_t hash_digest_size() const override
        {
            return 32;
        }

        void set_crypto(SshCrypto *crypto) override
        {
            crypto_ = crypto;
        }

        std::vector<uint8_t> generate_public_key() override
        {
            if (!crypto_)
                return {};
            std::vector<uint8_t> prime(dh_group14_prime, dh_group14_prime + sizeof(dh_group14_prime));
            std::vector<uint8_t> gen(dh_generator, dh_generator + sizeof(dh_generator));
            private_key_ = crypto_->dh_generate_key_pair(gen, prime, public_key_);
            return public_key_;
        }

        bool compute_shared_secret(const std::vector<uint8_t> &peer_public,
                                   std::vector<uint8_t> &shared_secret) override
        {
            if (!crypto_ || private_key_.empty())
                return false;
            std::vector<uint8_t> prime(dh_group14_prime, dh_group14_prime + sizeof(dh_group14_prime));
            shared_secret = crypto_->dh_compute_shared_secret(private_key_, peer_public, prime);
            return !shared_secret.empty();
        }

        std::vector<uint8_t> compute_exchange_hash(
            const std::string &client_version,
            const std::string &server_version,
            const std::vector<uint8_t> &client_kex_init,
            const std::vector<uint8_t> &server_kex_init,
            const std::vector<uint8_t> &host_key,
            const std::vector<uint8_t> &client_public,
            const std::vector<uint8_t> &server_public,
            const std::vector<uint8_t> &shared_secret) override
        {
            return SshKeyDerivation::compute_exchange_hash_sha256(
                client_version, server_version,
                client_kex_init, server_kex_init,
                host_key, client_public, server_public,
                shared_secret);
        }

        std::vector<uint8_t> public_key() const override
        {
            return public_key_;
        }

    private:
        SshCrypto *crypto_ = nullptr;
        std::vector<uint8_t> private_key_;
        std::vector<uint8_t> public_key_;
    };

    class SshKexDhGroup16Sha512 : public SshKexAlgorithm
    {
    public:
        std::string name() const override
        {
            return "diffie-hellman-group16-sha512";
        }
        size_t hash_digest_size() const override
        {
            return 64;
        }

        void set_crypto(SshCrypto *crypto) override
        {
            crypto_ = crypto;
        }

        std::vector<uint8_t> generate_public_key() override
        {
            if (!crypto_)
                return {};
            std::vector<uint8_t> prime(dh_group16_prime, dh_group16_prime + sizeof(dh_group16_prime));
            std::vector<uint8_t> gen(dh_generator, dh_generator + sizeof(dh_generator));
            private_key_ = crypto_->dh_generate_key_pair(gen, prime, public_key_);
            return public_key_;
        }

        bool compute_shared_secret(const std::vector<uint8_t> &peer_public,
                                   std::vector<uint8_t> &shared_secret) override
        {
            if (!crypto_ || private_key_.empty())
                return false;
            std::vector<uint8_t> prime(dh_group16_prime, dh_group16_prime + sizeof(dh_group16_prime));
            shared_secret = crypto_->dh_compute_shared_secret(private_key_, peer_public, prime);
            return !shared_secret.empty();
        }

        std::vector<uint8_t> compute_exchange_hash(
            const std::string &client_version,
            const std::string &server_version,
            const std::vector<uint8_t> &client_kex_init,
            const std::vector<uint8_t> &server_kex_init,
            const std::vector<uint8_t> &host_key,
            const std::vector<uint8_t> &client_public,
            const std::vector<uint8_t> &server_public,
            const std::vector<uint8_t> &shared_secret) override
        {
            return SshKeyDerivation::compute_exchange_hash_sha512(
                client_version, server_version,
                client_kex_init, server_kex_init,
                host_key, client_public, server_public,
                shared_secret);
        }

        std::vector<uint8_t> public_key() const override
        {
            return public_key_;
        }

    private:
        SshCrypto *crypto_ = nullptr;
        std::vector<uint8_t> private_key_;
        std::vector<uint8_t> public_key_;
    };

    class SshKexDhGroup18Sha512 : public SshKexAlgorithm
    {
    public:
        std::string name() const override
        {
            return "diffie-hellman-group18-sha512";
        }
        size_t hash_digest_size() const override
        {
            return 64;
        }

        void set_crypto(SshCrypto *crypto) override
        {
            crypto_ = crypto;
        }

        std::vector<uint8_t> generate_public_key() override
        {
            if (!crypto_)
                return {};
            std::vector<uint8_t> prime(dh_group18_prime, dh_group18_prime + sizeof(dh_group18_prime));
            std::vector<uint8_t> gen(dh_generator, dh_generator + sizeof(dh_generator));
            private_key_ = crypto_->dh_generate_key_pair(gen, prime, public_key_);
            return public_key_;
        }

        bool compute_shared_secret(const std::vector<uint8_t> &peer_public,
                                   std::vector<uint8_t> &shared_secret) override
        {
            if (!crypto_ || private_key_.empty())
                return false;
            std::vector<uint8_t> prime(dh_group18_prime, dh_group18_prime + sizeof(dh_group18_prime));
            shared_secret = crypto_->dh_compute_shared_secret(private_key_, peer_public, prime);
            return !shared_secret.empty();
        }

        std::vector<uint8_t> compute_exchange_hash(
            const std::string &client_version,
            const std::string &server_version,
            const std::vector<uint8_t> &client_kex_init,
            const std::vector<uint8_t> &server_kex_init,
            const std::vector<uint8_t> &host_key,
            const std::vector<uint8_t> &client_public,
            const std::vector<uint8_t> &server_public,
            const std::vector<uint8_t> &shared_secret) override
        {
            return SshKeyDerivation::compute_exchange_hash_sha512(
                client_version, server_version,
                client_kex_init, server_kex_init,
                host_key, client_public, server_public,
                shared_secret);
        }

        std::vector<uint8_t> public_key() const override
        {
            return public_key_;
        }

    private:
        SshCrypto *crypto_ = nullptr;
        std::vector<uint8_t> private_key_;
        std::vector<uint8_t> public_key_;
    };

    std::unique_ptr<SshKexAlgorithm> create_kex_dh_group14()
    {
        return std::make_unique<SshKexDhGroup14Sha256>();
    }

    std::unique_ptr<SshKexAlgorithm> create_kex_dh_group16()
    {
        return std::make_unique<SshKexDhGroup16Sha512>();
    }

    std::unique_ptr<SshKexAlgorithm> create_kex_dh_group18()
    {
        return std::make_unique<SshKexDhGroup18Sha512>();
    }
}
