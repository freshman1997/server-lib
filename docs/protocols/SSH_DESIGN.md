...
- Transport / Authentication / Connection / SFTP 主链路已实现并通过回归。
- 连接层已补齐 command 结束通知：支持 `exit-status` / `exit-signal`，并可由 `SshHandler::on_command_exit` 决定通知类型与字段。
- channel request 语义已增强：command 请求互斥、`env/pty-req` 时序校验、非 open 状态回复行为。
- SFTP 已加入路径安全加固（拒绝输入中的 `..` segment）及 realpath/readlink 逻辑路径一致性修复。
- PTY 进入分阶段接入：已具备伪终端分配、shell 子进程生命周期、基础数据桥接能力；`window-change/signal` 已预留桥接入口并模块化在 session 内部辅助函数。

### 1.5 第三方客户端兼容目标（新增）

- 目标客户端：OpenSSH / VSCode Remote-SSH / XShell。
- 兼容优先级：
  1. 认证链路（password/publickey）
  2. 交互链路（`pty-req` + `shell` + `window-change` + `signal`）
  3. 文件链路（SFTP v3 主路径）
  4. 可观测性（握手、认证、channel、退出状态日志）

### 1.6 自研客户端计划（新增）

- 阶段一（CLI）：`ssh` 交互终端 + 基础 SFTP 上传下载。
- 阶段二（GUI）：多会话 tab、终端窗口、SFTP 面板。
- 复用策略：优先复用当前 `SshTransport/SshMessageCodec`，避免协议栈重复实现导致行为漂移。

#### 1.6.1 当前 CLI 状态（2026-04 更新）

- 已提供 `ssh_cli` 模块，支持可插拔 transport：stub / loopback / process。
- 已提供 `SshCliShellSession` 会话句柄：`write/read/read_until/send_signal/is_alive/close`。
- 已提供 `ssh_cli_demo` 可执行程序：支持 `--cmd` 非交互探测与 `--interactive` 基础交互输入。
- 过程探测与本地集成测试具备 env-gate，默认不依赖外部环境。

### 1.7 可维护性与模块边界（新增）

- Session 层仅负责消息分发和状态流转，不承载 PTY 细节实现。
- PTY/终端桥接能力收敛在 `connection/ssh_terminal_bridge.*`：
  - shell 启动桥接
  - channel data <-> pty I/O
  - 退出序列（exit-status/exit-signal + eof + close）
  - window-change/signal 转发
- PTY 进程生命周期抽象收敛在 `connection/ssh_pty_process.*`，避免与协议编解码耦合。
