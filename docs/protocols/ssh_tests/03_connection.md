...
## 3.6 Interactive PTY Bridge (Step-by-step)
| # | Test Case | Input | Expected | Priority |
|---|-----------|-------|----------|----------|
| T3.6.1 | Shell starts PTY bridge | accepted `pty-req` then `shell` | PTY process created and bridge active | High |
| T3.6.2 | Channel data to PTY stdin | `CHANNEL_DATA` with shell command | PTY receives input and returns output via `CHANNEL_DATA` | High |
| T3.6.3 | Shell exit sequence | shell exits | emits `exit-status`, `CHANNEL_EOF`, `CHANNEL_CLOSE` | High |
| T3.6.4 | Window resize forwarding | `window-change` request | PTY winsize updated | Medium |
| T3.6.5 | Signal forwarding | `signal` request (`INT/TERM/KILL`) | signal delivered to PTY child process group | Medium |
