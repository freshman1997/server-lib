...
| T6.2.1a | OpenSSH interactive shell | `ssh -tt user@localhost` | PTY shell available, input/output normal | High |
...
| T6.2.3a | VSCode Remote-SSH | VSCode SSH extension | can connect and start remote server process | High |
| T6.2.3b | XShell terminal | XShell SSH terminal | interactive shell and resize work | High |
| T6.2.3c | ssh_cli_demo non-interactive | `ssh_cli_demo --cmd 'echo OK'` | receives command output | High |
| T6.2.3d | ssh_cli_demo interactive | `ssh_cli_demo --interactive` | shell write/read/signal/alive usable | High |
