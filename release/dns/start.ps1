...
$bin = Join-Path $resolvedBuildDir "release\dns\release_dns_server.exe"
if (-not (Test-Path -LiteralPath $bin)) {
    throw "release_dns_server.exe not found: $bin"
}

$resolvedConfig = $Config
if (-not $resolvedConfig -or $resolvedConfig.Trim().Length -eq 0) {
    if ($env:YUAN_DNS_CONFIG -and $env:YUAN_DNS_CONFIG.Trim().Length -gt 0) {
        $resolvedConfig = $env:YUAN_DNS_CONFIG
    } else {
...
if (-not $resolvedLog -or $resolvedLog.Trim().Length -eq 0) {
    $resolvedLog = Join-Path $PSScriptRoot "release_dns_server.log"
}
    ...
    if ($existing) {
        try {
            ...
            if ($null -ne $proc -and -not $proc.HasExited) {
                "release_dns_server is already running: $existing"
                exit 0
            }
        } catch {
        }
...
"release_dns_server started: $($proc.Id)"
"log: $resolvedLog"
"err: $resolvedErrLog"
