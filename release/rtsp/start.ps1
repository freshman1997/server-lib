...
$bin = Join-Path $resolvedBuildDir "release\rtsp\release_rtsp_server.exe"
if (-not (Test-Path -LiteralPath $bin)) {
    throw "release_rtsp_server.exe not found: $bin"
}

$resolvedConfig = $Config
if (-not $resolvedConfig -or $resolvedConfig.Trim().Length -eq 0) {
    if ($env:YUAN_RTSP_CONFIG -and $env:YUAN_RTSP_CONFIG.Trim().Length -gt 0) {
        $resolvedConfig = $env:YUAN_RTSP_CONFIG
    } else {
...
if (-not $resolvedLog -or $resolvedLog.Trim().Length -eq 0) {
    $resolvedLog = Join-Path $PSScriptRoot "release_rtsp_server.log"
}
...
if (-not $resolvedPid -or $resolvedPid.Trim().Length -eq 0) {
    $resolvedPid = Join-Path $PSScriptRoot "release_rtsp_server.pid"
}
    ...
    if ($existing) {
        try {
            ...
            if ($null -ne $proc -and -not $proc.HasExited) {
                "release_rtsp_server is already running: $existing"
                exit 0
            }
        } catch {
        }
...
"release_rtsp_server started: $($proc.Id)"
"log: $resolvedLog"
"err: $resolvedErrLog"
