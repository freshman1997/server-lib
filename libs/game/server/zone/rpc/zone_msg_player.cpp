...
namespace yuan::game::server
{
                                  ...
                                  ZoneMsgPlayerHandlers handlers)
    {
        ...
        const bool leave_registered = server.register_handler(game_route::zone_player_leave(), [address, handlers = std::move(handlers)](const yuan::rpc::Message &message) {
            ...
            const SSGatewayLoginRequest logout{0,
                                                role_id,
                                                address.service.pack(),
                                               gateway_session_id};
        ...
        const bool time_registered = server.register_handler(game_route::zone_time_sync(), [](const yuan::rpc::Message &message) {
            ...
            response.status = yuan::rpc::RpcStatus::ok;
            (void)encode_binary(CSTimeSyncResponse{true, request->role_id, request->client_time_seconds, receive_time_seconds, yuan::base::time::system_now_sec(), "time sync ok"},
                                response.payload);
            return response;
        });
