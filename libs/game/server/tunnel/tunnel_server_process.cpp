...
{
    if (argc != 2) {
        std::cerr << "usage: game_tunnel_server_process <config-file>\n";
        return 2;
    }
    ...
    auto service = std::make_shared<yuan::game::server::TunnelProcessService>(config->service_id, config->listen_port, config->expected_requests);
    if (!app.add_service("tunnel", service) || !app.start()) {
        std::cerr << "tunnel application failed to start\n";
        app.stop();
        return 3;
    }
