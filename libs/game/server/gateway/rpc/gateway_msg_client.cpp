...
namespace yuan::game::server
{
    namespace
    {
        ...
        yuan::rpc::Response handle_gateway_login(GatewayMsgContext &context,
                                                 GatewayZoneForwarder forward_to_zone,
                                                 const yuan::rpc::Message &message)
        {
            ...
            if (!login || login->status != yuan::rpc::RpcStatus::ok) {
                ...
                response.status = yuan::rpc::RpcStatus::internal_error;
                response.error = login ? (login->error.empty() ? "zone rejected gateway login" : login->error) : "gateway login failed";
                return response;
            }
        ...
        yuan::rpc::Response handle_gateway_forward(GatewayMsgContext &context,
                                                   GatewayZoneForwarder forward_to_zone,
                                                   ...
                                                   const yuan::rpc::Message &message)
        {
            ...
            const auto forwarded = forward_to_zone ? forward_to_zone(session->zone_service_id, std::move(zone_route), GatewayForwardContext{session->gateway_session_id, session->connection_id}, message.payload) : std::nullopt;
            if (!forwarded) {
                response.status = yuan::rpc::RpcStatus::unavailable;
                response.error = std::move(unavailable_error);
                return response;
            }
        ...
        yuan::rpc::Response handle_gateway_logout(GatewayMsgContext &context,
                                                  GatewayZoneForwarder forward_to_zone,
                                                  const yuan::rpc::Message &message)
        {
            auto response = make_response_for(message);
        ...
        yuan::rpc::Response handle_gateway_push(GatewayMsgContext &context,
                                                GatewayClientPusher push_to_client,
                                                const yuan::rpc::Message &message)
        {
            auto response = make_response_for(message);
        ...
        yuan::rpc::Response handle_gateway_session_close(GatewayMsgContext &context,
                                                         GatewaySessionCloser close_session,
                                                         const yuan::rpc::Message &message)
        {
            auto response = make_response_for(message);
            if (!has_internal_secret(message, context.gateway_internal_secret)) {
    ...
    {
        (void)select_login_zone;
        const bool login_registered = server.register_handler(game_route::gateway_login(), std::bind_front(handle_gateway_login, std::ref(context), forward_to_zone));
        const bool game_registered = server.register_handler(game_route::gateway_game_forward(), std::bind_front(handle_gateway_forward, std::ref(context), forward_to_zone, game_route::zone_echo(), "zone game request failed"));
        const bool logout_registered = server.register_handler(game_route::gateway_logout(), std::bind_front(handle_gateway_logout, std::ref(context), forward_to_zone));
        const bool time_registered = server.register_handler(game_route::gateway_time_sync(), std::bind_front(handle_gateway_forward, std::ref(context), forward_to_zone, game_route::zone_time_sync(), "zone time sync failed"));
        const bool push_registered = server.register_handler(game_route::gateway_push(), std::bind_front(handle_gateway_push, std::ref(context), push_to_client));
        const bool close_registered = server.register_handler(game_route::gateway_session_close(), std::bind_front(handle_gateway_session_close, std::ref(context), close_session));
