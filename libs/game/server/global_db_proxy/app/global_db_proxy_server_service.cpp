...
{
    GlobalDbProxyServerService::GlobalDbProxyServerService(ServiceServerConfig config)
        : service_id_(config.service_id),
          ...
          global_db_context_(ServiceAddress{service_id_, 700, yuan::game_base::ServerRole::world, service_id_.world, "global_db_proxy"})
    {
        ...
        tunnel_client_manager_.configure_registered_service(TunnelRegistration{service_id_.pack(), listen_host_, port_, "global_db_proxy"}, "global_db_proxy");
    }
    ...
    bool GlobalDbProxyServerService::init()
    {
        ...
        global_db_context_.resume_runtime = rpc_server_.runtime().runtime_view().raw();
