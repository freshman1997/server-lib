...
PluginPermission PluginPermissionNames::parse(const std::string &str)
{
    ...
    while (std::getline(iss, token, ',')) {
        ...
        if (token == "use_event_bus")        result = result | PluginPermission::use_event_bus;
        else if (token == "use_logger")      result = result | PluginPermission::use_logger;
        else if (token == "use_scheduler")   result = result | PluginPermission::use_scheduler;
        else if (token == "use_service_catalog")  result = result | PluginPermission::use_service_catalog;
        else if (token == "use_service_registry") result = result | PluginPermission::use_service_registry;
        else if (token == "use_http_intercept")   result = result | PluginPermission::use_http_intercept;
        else if (token == "use_storage")          result = result | PluginPermission::use_storage;
        else if (token == "all")             result = result | PluginPermission::all;
    }
    return result;
}
