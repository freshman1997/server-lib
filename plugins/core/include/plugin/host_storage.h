...
namespace yuan::plugin
{
    ...
    public:
        ...
        /// 获取 hash field
        virtual std::optional<std::string> hget(const std::string &key, const std::string &field) const = 0;

        /// 删除 hash field
        virtual bool hdel(const std::string &key, const std::string &field) = 0;

        /// 获取 hash 所有字段
        virtual std::unordered_map<std::string, std::string> hgetall(const std::string &key) const = 0;
